...
class OSS
{
     ...
     */
    public function __construct($isInternal = false, $config = [])
    {
        if (empty($config)) {
            $config = Config::get('oss.public');
        }
        $this->AccessKeyId = $config['access_key_id'];
        $this->AccessKeySecret = $config['access_key_secret'];
        $config['city'] && $this->city = $config['city'];
        $config['network_type'] && $this->networkType = $config['network_type'];
     ...
     * @throws Exception
     */
    public static function publicUploadToPublic($ossKey, $filePath, $options = [])
    {
        $oss = new OSS(false, Config::get('oss.public'));
        $oss->ossClient->setBucket(Config::get('oss.public.oss_root_path'));
     ...
     * @throws Exception
     */
    public static function publicUploadToPrivate($ossKey, $filePath, $options = [])
    {
        $oss = new OSS(false, Config::get('oss.private'));
        $oss->ossClient->setBucket(Config::get('oss.private.oss_root_path'));
        return $oss->ossClient->uploadFile($ossKey, $filePath, $options);
    }
    ...
    public static function getAllObjectKeyWithPrefix($bucketName, $folder_name, $nextMarker = '')
    {
        $oss = new OSS();
        return $oss->ossClient->getAllObjectKeyWithPrefix($bucketName, $folder_name, $nextMarker);
    }
