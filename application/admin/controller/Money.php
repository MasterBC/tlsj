...
class Money extends Base
{
    ...
    public function editConfig(Request $request)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                $info->_afterUpdate();
                AdminLog::addLog('修改钱包信息', $request->param());
            } catch (\Exception $e) {
            ...
            return view('money/edit_config', ['info' => $info]);
        }
    }
    ...
    public function userMoneyList(Request $request, Where $where)
    {
        ...
        if ($request->isAjax()) {
            ...
            return view('money/user_money_list', ['moneyNames' => $moneyNames]);
        }
    }
    ...
    public function editUserMoney(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('修改会员金额,会员id:' . $info['uid'] . ',钱包id:' . $info['mid'] . $str, $request->param());
            ...
            return view('money/edit_user_money', ['info' => $info]);
        }
    }
        ...
        if ($request->isPost()) {
            ...
            if ($money != 0) {
                ...
                try {
                    ...
                    AdminLog::addLog('拨出钱包金额, 会员id:' . $info['uid'] . ',钱包id:' . $info['mid'] . ',金额: ' . $money . ',备注:' . $note, $request->param());
            ...
            return view('money/out_user_money', ['info' => $info]);
        }
    }
    ...
    public function addLockMoney(Request $request, UsersMoney $userMoneyModel, AdminUser $adminUserModel)
    {
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('冻结会员金额,会员id:' . $info['uid'] . ',钱包id:' . $info['mid'] . ',金额:' . $money, $request->param());
            ...
            return view('money/add_lock_money', ['info' => $info]);
        }
    }
    ...
    public function lockMoneyLogList(Request $request, Where $where)
    {
        ...
        if ($request->isAjax()) {
            ...
            return view('money/lock_money_log_list', [
    ...
    public function releaseMoney(Request $request, UsersMoney $userMoneyModel, AdminUser $adminUserModel)
    {
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('释放会员冻结金额,日志id:' . implode(',', $id) . ', 成功id:' . implode(',', $successId), $request->param());
                $num = count($successId);
                Db::commit();
                if ($num > 0) {
    ...
    public function moneyLogList(Request $request, Where $where)
    {
        ...
        if ($request->isAJax()) {
            ...
            return view('money/money_log_list', [
    ...
    public function remittanceRechargeLogList(Request $request, Where $where, Users $userModel, Bank $bankModel)
    {
        if ($request->isAjax()) {
            try {
                ...
                $data = [
                    'code' => -1,
                    'msg' => '未获取到钱包充值记录，请联系管理员',
                ];
                return json()->data($data);
            }
        } else {
            return view('money/recharge/remittance_recharge_log_list');
        }
    }
    ...
    public function confirmRemittanceRecharge(Request $request, UsersMoneyAdd $usersMoneyAddModel)
    {
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('确认会员充值', $moneyAddInfo->toArray());
                Db::commit();
                return json()->data(['code' => 1, 'msg' => '操作成功']);
            } catch (\Exception $e) {
    ...
    public function refuseRemittanceRecharge(Request $request, UsersMoneyAdd $usersMoneyAddModel)
    {
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('拒绝会员充值', ['recharge_info' => $moneyAddInfo->toArray(), 'request_info' => $request->param()]);
                Db::commit();
                return json()->data(['code' => 1, 'msg' => '操作成功']);
            } catch (\Exception $e) {
    ...
    public function changeConfigList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('money/change/config_list');
        }
    ...
    public function editChangeConfig(Request $request, MoneyChange $moneyChangeModel)
    {
        ...
        if ($request->isPost()) {
            ...
            $moneyNames = MoneyModel::getMoneyNames();
            return view('money/change/edit_config', ['info' => $info, 'moneyNames' => $moneyNames]);
        }
    }
    ...
    public function changeLogList(Request $request, Where $where, Users $userModel)
    {
        if ($request->isAjax()) {
            ...
            return view('money/change/log_list');
        }
    }
    ...
    public function transformConfigList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('money/transform/config_list');
        }
    ...
    public function editTransformConfig(Request $request, MoneyTransform $moneyTransformModel)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                $info->_afterUpdate();
                AdminLog::addLog('修改钱包兑换参数', $request->param());
            } catch (\Exception $e) {
            ...
            $moneyNames = MoneyModel::getMoneyNames();
            return view('money/transform/edit_config', ['info' => $info, 'moneyNames' => $moneyNames]);
        }
    }
    ...
    public function transformLogList(Request $request, Where $where, Users $userModel)
    {
        if ($request->isAjax()) {
            ...
            return view('money/transform/log_list');
        }
    }
    ...
    public function carryConfigList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('money/carry/config_list');
        }
    }
    ...
    public function editCarryConfig(Request $request, MoneyCarry $moneyCarryModel)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                AdminLog::addLog('修改钱包提现参数', $request->param());
            } catch (\Exception $e) {
            ...
            $moneyNames = MoneyModel::getMoneyNames();
            return view('money/carry/edit_config', ['info' => $info, 'moneyNames' => $moneyNames]);
        }
    }
        ...
        if ($request->isAjax()) {
            try {
                ...
                foreach ($list as $v) {
                    ...
                    $arr['receipt_info'] = json_decode($v['receipt_info'], true);

                    $carryLogList[] = $arr;
                }
                if (count($carryLogList) == 0) {
                    $data = [
                        'code' => -1,
                        'msg' => '暂无记录'
                    ];
                ...
                $statusNum = MoneyCarryBankLog::group('status')->column('count(id)', 'status');
                $statusNum[0] = array_sum($statusNum);
            ...
            return view('money/carry/log_list');
        }
    }
    ...
    public function delCarryLog(Request $request)
    {
        if ($request->isAjax()) {
            ...
            try {
                ...
                if (count($carryList) <= 0) {
                    return json()->data(['code' => -1, 'msg' => '数据错误']);
                }
                $data = $carryList->toArray();
                foreach ($carryList as $v) {
                    $v->delete();
