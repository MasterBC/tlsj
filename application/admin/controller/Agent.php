    ...
    public function editConfig(Request $request)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                AdminLog::addLog('修改报单等级设置', $request->param());
            ...
            return view('agent/edit_config', ['info' => $info, 'levelNames' => get_level_name()]);
        }
    }
            ...
            try {
                ...
                foreach ($list as $v) {
                    $arr = [
                        ...
                        'old_level' => isset($agentNames[$v['front_id']]) ? $agentNames[$v['front_id']] : '',
                        'new_level' => isset($agentNames[$v['new_id']]) ? $agentNames[$v['new_id']] : '',
                ...
                $data = [
                    'code' => -1,
                    'msg' => '未获取到报单等级日志，请联系管理员',
                ];
                return json()->data($data);
            }
        } else {
            return view('agent/user_agent_log_list');
        }
    }
}