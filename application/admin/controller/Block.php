...
class Block extends Base
{
    ...
    public function editConfig(Request $request)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                $info->save();
                AdminLog::addLog('修改货币参数', $request->param());
            ...
            return view('block/edit_config', ['info' => $info]);
        }
    }
    ...
    public function blockLogList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('block/block_log_list');
        }
    }
    ...
    public function userBlockList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            try {
                ...
                foreach ($list as $v) {
                    $arr = [
                        'id' => $v['id'],
                        'account' => isset($users[$v['uid']]) ? $users[$v['uid']] : '',
                        'wallet_name' => get_block_name($v['bid']),
            ...
            return view('block/user_block_list', ['blockNames' => get_block_name()]);
        }
    }
    ...
    public function editUserBlock(Request $request, UsersBlock $userBlockModel)
    {
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('修改会员货币,会员id:' . $info['uid'] . ',货币id:' . $info['bid'] . $str, $request->param());
            ...
            return view('block/edit_user_block', ['info' => $info]);
        }
    }
    ...
    public function outUserBlock(Request $request, UsersBlock $userBlockModel, AdminUser $adminUserModel)
    {
        ...
        if ($request->isPost()) {
            ...
            if ($money != 0) {
                ...
                try {
                    if ($note == '') {
                        return json()->data(['code' => -1, 'msg' => '请输入备注']);
                    }
                    $userBlockModel->amountChange($info['uid'], $info['bid'], $money, 104, $note, ['admin_id' => $adminUserModel->getAdminUserId()]);

                    AdminLog::addLog('拨出货币金额, 会员id:' . $info['uid'] . ',货币id:' . $info['bid'] . ',金额: ' . $money . ',备注:' . $note, $request->param());
                ...
                return json()->data(['code' => -1, 'msg' => '没有金额变动']);
            }
        } else {
            return view('block/out_user_block', ['info' => $info]);
        }
    }
    ...
    public function addLockBlock(Request $request, UsersBlock $userBlockModel, AdminUser $adminUserModel)
    {
        ...
        if ($request->isPost()) {
            ...
            $note = $request->param('note', '', 'trim');
            if ($money <= 0) {
                return json()->data(['code' => -1, 'msg' => '冻结数量必须大于0']);
            }
            if ($note == '') {
                return json()->data(['code' => -1, 'msg' => '请输入备注']);
            }
            ...
            try {
                ...
                AdminLog::addLog('冻结会员金额,会员id:' . $info['uid'] . ',货币id:' . $info['bid'] . ',金额:' . $money, $request->param());
            ...
            return view('block/add_lock_block', ['info' => $info]);
        }
    }
    ...
    public function lockBlockLogList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('block/lock_block_log_list', ['blockLockLogTypes' => UsersBlockLockLog::getLogType(), 'blockNames' => BlockModel::getBlockNames()]);
        }
    }
    ...
    public function releaseBlock(Request $request, UsersBlockLockLog $userBlockLockLog, UsersBlock $userBlockModel, AdminUser $adminUserModel)
    {
        if ($request->isPost()) {
            ...
            try {
                ...
                if (empty($id)) {
                    return json()->data(['code' => -1, 'msg' => '请选择要执行数据']);
                }
                $note = $request->param('note', '', 'trim');
                ...
                AdminLog::addLog('释放会员冻结货币,日志id:' . implode(',', $id) . ', 成功id:' . implode(',', $successId), $request->param());
                $num = count($successId);
                Db::commit();
                if ($num > 0) {
                    return json()->data(['code' => 1, 'msg' => '成功释放' . $num . '条记录']);
                } else {
                    return json()->data(['code' => 1, 'msg' => '没有释放一条']);
                }
            } catch (\Exception $e) {
                Db::rollback();
                Log::write('释放货币冻结日志失败: （id: ' . implode(',', $id) . ', note: ' . $note . '）' . $e->getMessage(), 'error');
                return json()->data(['code' => -1, 'msg' => '释放失败']);
            }
        }
    }
    ...
    public function changeConfigList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('block/change_config_list');
        }
    ...
    public function editChangeConfig(Request $request, BlockChange $blockChangeModel)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                $info->_afterUpdate();
                AdminLog::addLog('修改货币转账参数', $request->param());
            } catch (\Exception $e) {
            ...
            $blockNames = get_block_name();
            return view('block/edit_change_config', ['info' => $info, 'blockNames' => $blockNames]);
        }
    }
    ...
    public function changeLogList(Request $request, Where $where, Users $userModel)
    {
        if ($request->isAjax()) {
            ...
            return view('block/change_log_list');
        }
    }
    ...
    public function transformConfigList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('block/transform_config_list');
        }
    }
     ...
     * 修改转换参数
     * @param Request $request
     * @param BlockTransformModel $blockTransformModel
     * @return \think\response\Json|\think\response\View
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
    ...
    public function editTransformConfig(Request $request, BlockTransform $blockTransformModel)
    {
        ...
        if ($request->isPost()) {
            try {
                ...
                $info->_afterUpdate();
                AdminLog::addLog('修改货币转换参数', $request->param());
            } catch (\Exception $e) {
            ...
            $blockNames = get_block_name();
            return view('block/edit_transform_config', ['info' => $info, 'blockNames' => $blockNames]);
        }
    }
    ...
    public function transformLogList(Request $request, Where $where, Users $userModel)
    {
        if ($request->isAjax()) {
            ...
            return view('block/transform_log_list');
        }
    }
    ...
    public function carryLogList(Request $request, Where $where)
    {
        if ($request->isAjax()) {
            ...
            return view('block/carry_log_list');
        }
    }
    ...
    public function confirmCarry(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            $res = $info->save();
            if ($res) {
                AdminLog::addLog('确认会员提现', $request->param());
                return json()->data(['code' => 1, 'msg' => '确认成功']);
            } else {
                return json()->data(['code' => -1, 'msg' => '确认失败']);
            }
        } else {
            return view('block/confirm_carry_info', [
                'info' => $info,
    ...
    public function refuseCarry(Request $request)
    {
        ...
        if ($request->isPost()) {
            if (!$content = $request->param('refuse_content', '', 'htmlspecialchars')) {
                return json()->data(['code' => -1, 'msg' => '请输入拒绝理由']);
            }
            $info->status = 3;
            ...
            $res = $info->save();
            if ($res) {
                AdminLog::addLog('拒绝会员提现', $request->param());
                return json()->data(['code' => 1, 'msg' => '拒绝成功']);
            } else {
                return json()->data(['code' => -1, 'msg' => '拒绝失败']);
            }
        } else {
            return view('block/refuse_carry_info', [
                'info' => $info,
                'status' => BlockCarry::$carryStatus
            ]);
        }
    ...
    public function delCarryLog(Request $request)
    {
        if ($request->isPost()) {
            ...
            if ($id == '') {
                return Json::create(['code' => -1, 'msg' => '请选择要删除的数据']);
            }
            try {
                $id = explode(',', $id);
                $logList = BlockCarry::whereIn('id', $id)->select()->toArray();
                BlockCarry::whereIn('id', $id)->delete();
                AdminLog::addLog('删除会员提现记录', $logList);
                return Json::create(['code' => 1, 'msg' => '删除成功']);
            } catch (\Exception $e) {
                return Json::create(['code' => -1, 'msg' => '删除失败']);
            }
        }
        ...
        if ($request->isAjax()) {
            try {
                ...
                foreach ($list as $v) {
                    ...
                    foreach ($arr['img'] as $key => $val) {
                        $arr['img'][$key] = get_img_domain().$val;
                    }
                    $logList[] = $arr;
                }
                if (count($logList) == 0) {
                ...
                $data = [
                    'code' => -1,
                    'msg' => '未获取到货币充值记录，请联系管理员',
                ];
                return json()->data($data);
            }
        } else {
            return view('block/recharge_log_list');
        }
    }
    ...
    public function confirmRecharge(Request $request)
    {
        if ($request->isPost()) {
            ...
            if (empty($info) || $info['status'] != 1) {
                return Json::create(['code' => -1, 'msg' => '此信息不支持操作，请刷新后重试']);
            }
            try {
                $info->status = 9;
                ...
                AdminLog::addLog('确认会员充值', $info->toArray());

                return Json::create(['code' => 1, 'msg' => '操作成功']);
            } catch (\Exception $e) {
                Log::write('确认汇款充值失败: ' . $e->getMessage(), 'error');
                return Json::create(['code' => -1, 'msg' => '操作失败']);
            }
        }
    }
    ...
    public function refuseRecharge(Request $request)
    {
        if ($request->isPost()) {
            ...
            if ($refuseContent == '') {
                return Json::create(['code' => -1, 'msg' => '请输入拒绝理由']);
            }
            try {
                $info->status = 3;
                ...
                AdminLog::addLog('拒绝会员充值', $info->toArray());
    ...
    public function delRechargeLog(Request $request)
    {
        if ($request->isPost()) {
            ...
            try {
                ...
                AdminLog::addLog('删除会员充值记录', $logList);
                return Json::create(['code' => 1, 'msg' => '删除成功']);
            } catch (\Exception $e) {
                return Json::create(['code' => -1, 'msg' => '删除失败']);
            }
